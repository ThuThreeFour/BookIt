...
function reserveAppointment() {
  ...
  if (file_exists($file)) {
    ...
    file_put_contents($file, json_encode(buildDefaultMonth()));
...
function buildDefaultMonth() {
  ...
  //$totalDays = array(31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31);
  $json = array();
...
function writeDefaultMonth($month, $year) {
  $file = "json/$month" . "_" . "$year.json";
  file_put_contents($file, json_encode(buildDefaultMonth(), JSON_PRETTY_PRINT));
  return;
  //header('Location: home.html');
}
